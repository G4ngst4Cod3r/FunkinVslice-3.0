...
class PlayState extends MusicBeatState
{
		...
		var noteComboNumbers:FlxSprite;
		...
		var defaultBoyfriendY:Float = 450;
		...
		var protoEngineVer:String = ' (89350fe) ';
		var ver = Application.current.meta.get('version');
		var compiledversion = " Prototype ";
		...
		override public function create()
		{
			...
			//missTxt = new FlxText(healthBarBG.x + healthBarBG.width - 190, healthBarBG.y + 30, 0, "", 20);
			//missTxt.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, RIGHT, OUTLINE, FlxColor.BLACK);
			//missTxt.scrollFactor.set();
			//add(missTxt);

			if (!PreferencesMenu.getPref('hideproto'))
			{
				prototypeText = new FlxText(980, 700, "v" + ver + protoEngineVer + compiledversion,  12);
			...
			noteCombo = new FlxSprite().loadGraphic(Paths.image('noteCombo'));
			noteCombo.frames = Paths.getSparrowAtlas('noteCombo');
			noteCombo.scale.set(0.6, 0.6);
			noteCombo.visible = true;
			noteCombo.alpha = 0.00001;
			noteCombo.animation.addByPrefix('appear', 'NoteCombofix', 24, false);
			add(noteCombo);
		

			var x = defaultBoyfriendX / 10 + boyfriend.x / 4;
			var y = defaultBoyfriendY / 10 + boyfriend.y / 6 + 300;
			...
			function ughIntro()
			{
				...
				video.playVideo('assets/music/ughCutscene.mp4');
			...
			function gunsIntro()
			{
				...
				#if desktop
				var video:VideoHandler = new VideoHandler();
				video.playVideo('assets/music/gunsCutscene.mp4');
				video.finishCallback = function()
				{
					FlxTween.tween(FlxG.camera, {zoom: defaultCamZoom}, (Conductor.crochet / 1000) * 5, {ease: FlxEase.quadInOut});
					startCountdown();
			...
			function stressIntro()
			{
				...
				#if desktop
				var video:VideoHandler = new VideoHandler();
				video.playVideo('assets/music/stressCutscene.mp4');
				video.finishCallback = function()
				{
					FlxTween.tween(FlxG.camera, {zoom: defaultCamZoom}, (Conductor.crochet / 1000) * 5, {ease: FlxEase.quadInOut});
					startCountdown();
			...
			function darnellIntro()
			{
				#if desktop
				var video:VideoHandler = new VideoHandler();
				video.playVideo('assets/music/week8trler.mp4');
				video.finishCallback = function()
				{
					FlxTween.tween(FlxG.camera, {zoom: defaultCamZoom}, (Conductor.crochet / 1000) * 5, {ease: FlxEase.quadInOut});
					startCountdown();
				...
				var vid:FlxVideo = new FlxVideo('music/week8trler.mp4');
				vid.finishCallback = function()
				{
					remove(blackShit);
		...
		override public function update(elapsed:Float)
		{
			if (PlayState.SONG.notes[Std.int(curStep / 16)] != null) {
				...
				var loop:Int = 1;
				var seperatedNoteCombo:Array<String> = Std.string(noteHits).split("");
				if(noteHits > 0){
					for (i in seperatedNoteCombo){
						noteComboNumbers = new FlxSprite().loadGraphic(Paths.image('noteComboNumbers'));
						noteComboNumbers.frames = Paths.getSparrowAtlas('noteComboNumbers');
						noteComboNumbers.scale.set(0.6, 0.6);
						noteComboNumbers.visible = true;
						noteComboNumbers.updateHitbox();
						noteComboNumbers.x = boyfriend.x+ boyfriend.width - noteCombo.width + (loop * 100) + 200; 
						noteComboNumbers.x -= 90 * (seperatedNoteCombo.length - 1);
						noteComboNumbers.y = (boyfriend.y + boyfriend.height) / 4 + 112.5 - (loop * 25);
						noteComboNumbers.animation.addByPrefix('appear', i + '_appear', 24, false);
						noteComboNumbers.animation.addByPrefix('disappear', i + '_disappear', 24, false);
						add(noteComboNumbers);
						spritenoob.push(noteComboNumbers);
						noteComboNumbers.animation.play('appear');
						loop++;
					}
					...
					noteHits = 0;
					noteCombo.x = boyfriend.x + boyfriend.width- noteCombo.width;
					noteCombo.y = (boyfriend.y + boyfriend.height) / 4;
					noteCombo.alpha = 1;
					noteCombo.visible = true;
